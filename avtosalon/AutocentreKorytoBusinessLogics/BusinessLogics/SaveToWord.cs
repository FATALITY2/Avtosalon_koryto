...
("Название: " + purchase.PurchaseName, new WordTextProperties {Bold = true, Size = "24", }), (" || Сумма покупки: " + purchase.PurchaseSum.ToString(), new WordTextProperties {Bold = false, Size = "24", }) },
        ...
        public static void CreateDocCar(WordInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                MainDocumentPart mainPart = wordDocument.AddMainDocumentPart();
                ...
                foreach (var car in info.Cars)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordTextProperties)> {("Название: " + car.CarName, new WordTextProperties {Bold = true, Size = "24", }), 
                            (" Материал: " + car.Equipment.ToString() + " Цена: " + car.CarPrice, new WordTextProperties {Bold = false, Size = "24", }) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
                            JustificationValues = JustificationValues.Both
